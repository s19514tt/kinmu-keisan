<template>
  <div>
    ...
    <v-card class="ma-4 pa-4">
      ...
      <div>{{ chingin }}</div>
      <div style="white-space: break-spaces;" v-text="chinginlist"></div>
      <div>
  ...
  methods: {
    ...
    calChingin: async function () {
      ...
      if (filteredWorkData.length !== 0) {
        ...
        this.chinginlist = filteredWorkData.map((item) => `勤務開始:${new Date(item.start).toLocaleString()} 勤務終了:${new Date(item.finish).toLocaleString()} 休憩:${item.kyukei}分 勤務分数:${(new Date(item.finish).getTime() - new Date(item.start).getTime()) / 60000 - item.kyukei}分`).join('\n')
